// js/estadisticas.js - REPORTES Y KPIs DE VENTAS (V6.3 - CON FUNCIÓN DE CORTE DE CAJA)
document.addEventListener('DOMContentLoaded', async () => {
    ...
    // --- 8. FUNCIÓN DE CORTE DE CAJA (NUEVA) ---
    window.realizarCorteCaja = async () => {
        if (ventasHoy.length === 0) return alert("No hay ventas para realizar el corte.");

        const confirmar = confirm("¿Deseas imprimir el corte y reiniciar los totales del día?");
        if (!confirmar) return;

        // 1️⃣ Imprimir primero
        window.imprimirCorteCaja();

        // 2️⃣ Registrar el corte (opcional, si tienes tabla 'cortes_caja')
        try {
            const { error } = await db
                ...
                .insert({
                    ...
                    total: ventasHoy.reduce((acc, v) => acc + parseFloat(v.total || 0), 0),
                    total_efectivo: ventasHoy.filter(v => (v.metodo_pago || '').toLowerCase().includes('efectivo')).reduce((a, v) => a + parseFloat(v.total || 0), 0),
                    total_tarjeta: ventasHoy.filter(v => (v.metodo_pago || '').toLowerCase().includes('tarjeta')).reduce((a, v) => a + parseFloat(v.total || 0), 0),
                    num_ventas: ventasHoy.length,
                    usuario: sesion?.nombre || 'Desconocido'
                });
            if (error) console.warn("Aviso: no se pudo registrar el corte en la tabla cortes_caja:", error.message);
        } catch (err) {
            console.warn("Error al registrar corte:", err.message);
        }

        // 3️⃣ Limpiar totales en pantalla (caja visualmente en cero)
        ventasHoy = [];
        actualizarKPIs();
        renderizarTabla();
        if (chartInstancia && chartInstancia.destroy) chartInstancia.destroy();

        alert("✅ Corte realizado correctamente. Caja lista para el siguiente turno.");
    };
